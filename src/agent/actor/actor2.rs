    ...
    fn handle_internal(
        ...
        match &message {
            ...
            ForkMessage::UsingFork(who) => match &state.owned_by {
                ...
                None => {
                    log::info!("ERROR Received `UsingFork` from {who:?}. Real owner is `None`");
                }
            },
            ForkMessage::PutForkDown(who) => match &state.owned_by {
                ...
                None => {
                    log::info!("ERROR Received `PutForkDown` from {who:?}. Real owner is `None`");
                }
            },
        }
        None
...
mod tests {
    ...
    async fn test_multi_actor() {
        ...
        for left in 0..philosopher_names.len() {
            ...
            let p: PhilosopherArguments = PhilosopherArguments {
                time_slice,
